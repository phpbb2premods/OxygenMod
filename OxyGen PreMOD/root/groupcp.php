...
//-- mod : online offline hidden -----------------------------------------------
//-- add
	global $online_color, $offline_color, $hidden_color;
//-- fin mod : online offline hidden -------------------------------------------
...
MOD-*/
//-- add
	$temp_url = $get->url('profile', array('mode' => 'viewprofile', POST_USERS_URL => $row['user_id']), true);
//-- fin mod : rank color system -----------------------------------------------
...
//-- add
if( $board_config['groups_access'] == ADMIN && $userdata['user_level'] != ADMIN )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
else if ( $board_config['groups_access'] == MOD && ( $userdata['user_level'] != MOD && $userdata['user_level'] != ADMIN ) )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
else if( $board_config['groups_access'] == USER && !$userdata['session_logged_in'] )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
...
//-- mod : online offline hidden -----------------------------------------------
//-- add
	$sql = str_replace('SELECT ', 'SELECT u.user_allow_viewonline, u.user_session_time, ', $sql);
//-- fin mod : online offline hidden -------------------------------------------
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Error getting user list for group', '', __LINE__, __FILE__, $sql);
...
MOD-*/
//-- add
		'U_MOD_VIEWPROFILE' => $get->url('profile', array('mode' => 'viewprofile', POST_USERS_URL => $user_id), true),
//-- fin mod : rank color system -----------------------------------------------
...
MOD-*/
//-- add
				'U_VIEWPROFILE' => $get->url('profile', array('mode' => 'viewprofile', POST_USERS_URL => $user_id), true),
			));
//-- fin mod : rank color system -----------------------------------------------
