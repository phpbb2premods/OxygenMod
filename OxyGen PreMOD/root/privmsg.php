...
//	&& $folder != 'trashbox' 
//-- modify
	if ( $folder != 'inbox' && $folder != 'outbox' && $folder != 'sentbox' && $folder != 'savebox' && $folder != 'trashbox' )
//-- fin mod : trashbox --------------------------------------------------------
	{
		$folder = 'inbox';
	}
...
//-- mod : pm threshold --------------------------------------------------------
//-- add
if ( !$userdata['session_logged_in'] || ($userdata['user_posts'] < $board_config['pm_allow_threshold']) )
{
...
if ($userdata['user_level'] == ADMIN)
{
	...
	$board_config['max_trashbox_privmsgs'] = $board_config['administrator_max_trashbox_privmsgs'];
}
elseif ($userdata['user_level'] == MOD)
{
	...
	$board_config['max_trashbox_privmsgs'] = $board_config['moderator_max_trashbox_privmsgs'];
}
...
//-- mod : trashbox ------------------------------------------------------------
//-- add
$trashbox = intval($board_config['max_trashbox_privmsgs']) > 0;
if ( $trashbox )
{
	if ( $folder != 'outbox' )
	{
	...
	$template->assign_block_vars('switch_trashbox', array());
}
else if ( $folder == 'trashbox' )
{
	$folder = 'inbox';
}

if ( $folder != 'outbox' && $folder != 'savebox' )
{
	$template->assign_block_vars("switch_save_messages", array());
}
//-- fin mod : trashbox --------------------------------------------------------
...
//
$inbox_img = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '"><img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '" title="' . $lang['Inbox'] . '" /></a>' : '<img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '" />';
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];

$outbox_img = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '"><img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '" title="' . $lang['Outbox'] . '" /></a>' : '<img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '" />';
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];

$sentbox_img = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '"><img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '" title="' . $lang['Sentbox'] . '" /></a>' : '<img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '" />';
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : $lang['Sentbox'];

$savebox_img = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '"><img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '" title="' . $lang['Savebox'] . '" /></a>' : '<img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '" />';
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : $lang['Savebox'];

//-- mod : trashbox ------------------------------------------------------------
//-- add
$trashbox_img = ( $folder != 'trashbox' || $mode != '' ) ? '<a href="' . append_sid('privmsg.'.$phpEx . '?folder=trashbox') . '"><img src="' . $images['pm_trashbox'] . '" border="0" alt="' . $lang['Trashbox'] . '" title="' . $lang['Trashbox'] . '" /></a>' : '<img src="' . $images['pm_trashbox'] . '" border="0" alt="' . $lang['trashbox'] . '" title="' . $lang['Trashbox'] . '" />';
$trashbox_url = ( $folder != 'trashbox' || $mode != '' ) ? '<a href="' . append_sid('privmsg.'.$phpEx . '?folder=trashbox') . '">' . $lang['Trashbox'] . '</a>' : $lang['Trashbox'];
//-- fin mod : trashbox --------------------------------------------------------

//-- mod : export pms to xml ---------------------------------------------------
//-- add
$export_img = '<a href="' . append_sid('export_privmsgs.'.$phpEx) . '"><img src="' . $images['pm_export'] . '" border="0" alt="' . $lang['Export'] . '" title="' . $lang['Export'] . '" /></a>';
$export_url = '<a href="' . append_sid('export_privmsgs.'.$phpEx) . '">' . $lang['Export'] . '</a>';
//-- fin mod : export pms to xml -----------------------------------------------
...
//-- add
		case 'trashbox':
			$l_box_name = $lang['Trashbox'];
			$pm_sql_user = "AND ( ( pm.privmsgs_to_userid = " . $userdata['user_id'] . "
					AND pm.privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL . " ) 
				OR ( pm.privmsgs_from_userid = " . $userdata['user_id'] . "
					AND pm.privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL . " ) 
				)";
			break;
//-- fin mod : trashbox --------------------------------------------------------
		default:
...
//-- add
	$sql = str_replace(', u.user_id AS user_id_1', ', u.user_id AS user_id_1, u.user_allow_viewonline AS user_allow_viewonline_1, u.user_session_time AS user_session_time_1', $sql);
	$sql = str_replace(', u2.user_id AS user_id_2', ', u2.user_id AS user_id_2, u2.user_allow_viewonline AS user_allow_viewonline_2, u2.user_session_time AS user_session_time_2', $sql);
...
//-- add
	...
	{
		if ( $privmsg['privmsgs_type'] == PRIVMSGS_TRASH_IN_MAIL )
		{
			$post_img = $post_icons['post_img'];
			$reply_img = $post_icons['reply_img'];
			$quote_img = $post_icons['quote_img'];
		...
		$l_box_name = $lang['Privmsg_Deleted'];
	}
//-- fin mod : trashbox --------------------------------------------------------
...
//-- mod : trashbox ------------------------------------------------------------
//-- add
		'L_TRASHBOX' => $lang['Privmsg_Deleted'],
//-- fin mod : trashbox --------------------------------------------------------
		'L_FLAG' => $lang['Flag'],
...
MOD-*/
//-- add
		'L_SAVE_MSG' => ( $folder != 'trashbox' ) ? $lang['Save_message'] : $lang['Undelete_message'], 
		'L_DELETE_MSG' => ( $folder != 'trashbox' ) ? $lang['Delete_message'] : $lang['Purge_message'], 
//-- fin mod : trashbox --------------------------------------------------------
...
MOD-*/
//-- add
	$temp_url = $get->url('profile', array('mode' => 'viewprofile', POST_USERS_URL => $user_id_from), true);
//-- fin mod : rank color system -----------------------------------------------
...
//-- add
			case 'trashbox':
				$delete_type = "( ( privmsgs_from_userid = " . $userdata['user_id'] . " 
					AND privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL . " ) 
				OR ( privmsgs_to_userid = " . $userdata['user_id'] . " 
					AND privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL . " ) )";
				break;
//-- fin mod : trashbox --------------------------------------------------------
		}
...
//-- add
			if ( $folder != 'trashbox' )
			{
				// See if recipient is at their trashbox limit
				$sql = "SELECT COUNT(privmsgs_id) AS trashbox_items, MIN(privmsgs_date) AS oldest_post_time 
					FROM " . PRIVMSGS_TABLE . " 
				...
				if ( $trash_info = $db->sql_fetchrow($result) )
				{
					if ( $trash_info['trashbox_items'] >= $board_config['max_trashbox_privmsgs'] )
					{
						$sql = "SELECT privmsgs_id FROM " . PRIVMSGS_TABLE . " 
							WHERE ( ( privmsgs_to_userid = " . $userdata['user_id'] . " 
										AND privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL . " )
									OR ( privmsgs_from_userid = " . $userdata['user_id'] . " 
										AND privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL . ") ) 
								AND privmsgs_date = " . $trash_info['oldest_post_time'];
						if ( !$result = $db->sql_query($sql) )
						{
							message_die(GENERAL_ERROR, 'Could not find oldest privmsgs (delete)', '', __LINE__, __FILE__, $sql);
						}
						$old_privmsgs_id = $db->sql_fetchrow($result);
						$old_privmsgs_id = $old_privmsgs_id['privmsgs_id'];
						...
						if ( !$db->sql_query($sql) )
						{
							message_die(GENERAL_ERROR, 'Could not delete oldest privmsgs (delete)', '', __LINE__, __FILE__, $sql);
						}
						...
						if ( !$db->sql_query($sql) )
						{
							message_die(GENERAL_ERROR, 'Could not delete oldest privmsgs text (delete)', '', __LINE__, __FILE__, $sql);
						}
					}
				}

				// Process request
				$delete_sql = "UPDATE " . PRIVMSGS_TABLE;
				$delete_sql2 = "UPDATE " . PRIVMSGS_TABLE;

				switch ($folder)
				{
					case 'inbox':
						$delete_sql .= " SET privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL . " 
							WHERE privmsgs_to_userid = " . $userdata['user_id'] . " 
					...
					case 'outbox':
						$delete_sql .= " SET privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL . " 
					...
					case 'savebox':
						$delete_sql .= " SET privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL . " 
							WHERE privmsgs_to_userid = " . $userdata['user_id'] . " 
								AND privmsgs_type = " . PRIVMSGS_SAVED_IN_MAIL;
						$delete_sql2 .= " SET privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL . " 
							WHERE privmsgs_from_userid = " . $userdata['user_id'] . " 
								AND privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL;
						break;
				}

				$delete_sql .= " AND privmsgs_id IN ($delete_sql_id)";
				$delete_sql2 .= " AND privmsgs_id IN ($delete_sql_id)";

				if ( !$db->sql_query($delete_sql) )
				{
					message_die(GENERAL_ERROR, 'Could not delete private messages', '', __LINE__, __FILE__, $delete_sql);
				}

				if ( $folder == 'savebox' && !$db->sql_query($delete_sql2) )
				{
					message_die(GENERAL_ERROR, 'Could not delete private messages', '', __LINE__, __FILE__, $delete_sql2);
				}
			}
			
			if ( $folder == 'outbox' || $folder == 'trashbox' || !$trashbox )
			{
				// Delete the messages
				$delete_text_sql = "DELETE FROM " . PRIVMSGS_TEXT_TABLE . "
					WHERE privmsgs_text_id IN ($delete_sql_id)";
...
//-- mod : trashbox ------------------------------------------------------------
//-- add
		$saved_sql2 = "UPDATE " . PRIVMSGS_TABLE;
//-- fin mod : trashbox --------------------------------------------------------
...
//-- add
			case 'trashbox':
				$saved_sql .= " SET privmsgs_type = " . PRIVMSGS_SAVED_IN_MAIL . " 
					WHERE privmsgs_to_userid = " . $userdata['user_id'] . " 
						AND privmsgs_type = " . PRIVMSGS_TRASH_IN_MAIL;
				$saved_sql2 .= " SET privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . " 
					WHERE privmsgs_from_userid = " . $userdata['user_id'] . " 
						AND privmsgs_type = " . PRIVMSGS_TRASH_OUT_MAIL;
				break;
//-- fin mod : trashbox --------------------------------------------------------
...
//-- mod : trashbox ------------------------------------------------------------
//-- add
		$saved_sql2 .= " AND privmsgs_id IN ($saved_sql_id)";
//-- fin mod : trashbox --------------------------------------------------------
...
//-- mod : trashbox ------------------------------------------------------------
//-- add
		if ( $folder == 'trashbox' && !$db->sql_query($saved_sql2) )
		{
			message_die(GENERAL_ERROR, 'Could not save private messages', '', __LINE__, __FILE__, $saved_sql2);
		}
//-- fin mod : trashbox --------------------------------------------------------
...
*/
//-- add
			$privmsg_message = str_replace(array('<', '>', '"'), array('&lt;', '&gt;', '&quot;'), $privmsg_message);
//-- fin mod : categories hierarchy --------------------------------------------
...
//-- fin mod : view pm while replying ------------------------------------------
		}
		else
		{
...
MOD-*/
//-- add
		$preview_message = stripslashes(prepare_message(addslashes(unprepare_message($privmsg_message)), $html_on, $bbcode_on, $smilies_on, $bbcode_uid));
//-- fin mod : non-persistent XSS issue ----------------------------------------
...
// New message
//
$post_new_mesg_url = '<a href="' . append_sid("privmsg.$phpEx?mode=post") . '"><img src="' . $images['post_new'] . '" alt="' . $lang['Send_a_new_message'] . '" title="' . $lang['Send_a_new_message'] . '" border="0" /></a>';
...
//-- add
		case 'trashbox':
			$l_box_size_status = sprintf($lang['Trashbox_size'], $inbox_limit_pct);
			break;
//-- fin mod : trashbox --------------------------------------------------------
		default:
			$l_box_size_status = '';
...
MOD-*/
//-- add
	'L_FROM_OR_TO' => ( $folder == 'inbox' || $folder == 'savebox' || $folder == 'trashbox' ) ? $lang['From'] : $lang['To'], 
//-- fin mod : trashbox --------------------------------------------------------
...
MOD-*/
//-- add
	'L_DELETE_MARKED' => ( $folder != 'trashbox' ) ? $lang['Delete_marked'] : $lang['Purge_marked'], 
	'L_DELETE_ALL' => ( $folder != 'trashbox' ) ? $lang['Delete_all'] : $lang['Purge_all'], 
	'L_SAVE_MARKED' => ( $folder != 'trashbox' ) ? $lang['Save_marked'] : $lang['Undelete_marked'], 
//-- fin mod : trashbox --------------------------------------------------------
...
MOD-*/
//-- add
		$u_from_user_profile = $get->url('profile', array('mode' => 'viewprofile', POST_USERS_URL => $msg_userid), true);
//-- fin mod : rank color system -----------------------------------------------
