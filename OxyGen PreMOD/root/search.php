...
//-- add
if( $board_config['search_access'] == ADMIN && $userdata['user_level'] != ADMIN )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
else if ( $board_config['search_access'] == MOD && ( $userdata['user_level'] != MOD && $userdata['user_level'] != ADMIN ) )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
else if( $board_config['search_access'] == USER && !$userdata['session_logged_in'] )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Authorised']);
}
...
MOD-*/
//-- add
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' || $search_id == 'egotopicsearch' || $search_id == 'usertopics' )
...
MOD-*/
//-- add
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'egotopicsearch' || $search_id == 'usertopics' || ( $search_author != '' && $search_keywords == '' )  )
//-- fin mod : topics a user has started ---------------------------------------
		{
...
//-- add
		...
		if ($show_results == 'topics')
		{
			$sql = str_replace(', u2.user_id as id2', ', u2.user_id as id2, u2.user_cf_iso3661_1 as user2flag', $sql);
		}
...
//-- add
			colorize_title($searchset[$i]['topic_title'], $searchset[$i]['topic_attribute'], $searchset[$i]['topic_attribute_position'], $searchset[$i]['topic_attribute_color'], $searchset[$i]['topic_attribute_username'], $searchset[$i]['topic_attribute_date']);
...
//-- mod : smiley categories ---------------------------------------------------
//-- add
			$forum_cat_id = $searchset[$i]['cat_id'];
//-- fin mod : smiley categories -----------------------------------------------
...
//-- mod : post description ----------------------------------------------------
//-- add
					$post_sub_title = !empty($searchset[$i]['post_sub_title']) ? ( count($orig_word) ? preg_replace($orig_word, $replacement_word, $searchset[$i]['post_sub_title']) : $searchset[$i]['post_sub_title'] ) : '';
//-- fin mod : post description ------------------------------------------------
...
//-- mod : ip country flag -----------------------------------------------------
//-- add
				$topic_author_ipcf = $searchset[$i]['user_cf_iso3661_1'];
				$lastpost_iso3661_1 = $searchset[$i]['user2flag'];
//-- fin mod : ip country flag -------------------------------------------------
...
//-- mod : hypercell class -----------------------------------------------------
//-- add
					'HYPERCELL_CLASS' => get_hypercell_class($searchset[$i]['topic_status'], ($folder_image == $folder_new), $searchset[$i]['topic_type'], $replies),
