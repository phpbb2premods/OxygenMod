...
//-- add
$user_ip_real = encode_ip(ARIHT_getipreelle());
...
//-- mod : the logger ----------------------------------------------------------
//-- add
include( "{$phpbb_root_path}includes/class_log.$phpEx" );
$log = new log_handler();
//-- fin mod : the logger ------------------------------------------------------

//-- mod : auto un-ban main admin ----------------------------------------------
//-- add
$sql = 'DELETE FROM ' . BANLIST_TABLE . ' WHERE ban_userid = 2';
if (!$db->sql_query($sql))
{
	message_die(GENERAL_MESSAGE, 'Unable to access the banlist table');
}
//-- fin mod : auto un-ban main admin ------------------------------------------
