...
//-- add
	$bbcode_tpl['php_open'] = str_replace('{L_PHP}', $lang['PHPCode'], $bbcode_tpl['php_open']);
...
//-- mod : php bbcode box reloaded ---------------------------------------------
//-- add
	$expand_ary3 = array('{L_EXPAND}', '{L_EXPAND_MORE}', '{L_CONTRACT}', '{L_SELECT_ALL}');
	$expand_ary4 = array($lang['bbcbxr_expand'], $lang['bbcbxr_expand_more'], $lang['bbcbxr_contract'], $lang['bbcbxr_select']);
	$bbcode_tpl['php_open'] = str_replace($expand_ary3, $expand_ary4, $bbcode_tpl['php_open']);
//-- fin mod : php bbcode box reloaded -----------------------------------------
...
//-- add
	...
	$youtube = $bbc_tag['youtube'];
	$patterns[] = "#\[$youtube:$uid\]http://(?:www\.)?youtube.com/watch\?v=([0-9A-Za-z-_]{11})[^[]*\[/$youtube:$uid\]#is";
...
//-- add
	// [youtube]YouTube URL[/youtube] code..
	$youtube = $bbc_tag['youtube'];
	$bbc_sort = bbc_auth($bbc_auth['youtube']);
	$text = ($bbc_value['youtube'] && $bbc_sort) ? preg_replace("#\[$youtube\]http://(?:www\.)?youtube.com/watch\?v=([0-9A-Za-z-_]{11})[^[]*\[/$youtube\]#is", "[$youtube:$uid]http://youtube.com/watch?v=\\1[/$youtube:$uid]", $text) : $text;
//-- fin mod : youtube for bbcbxr ----------------------------------------------
...
//-- add
	...
	$replacements[] = "\\1<a href=\"\\2\\3\\4\\5\" target=\"_blank\">\\2...\\4\\5</a>";
	$ret = preg_replace($patterns, $replacements, $ret);
//-- fin mod : auto shorten urls -----------------------------------------------
...
//-- add
	...
	$replazy[] = "\\1<a href=\"http://\\2\\4\" target=\"_blank\">\\2\\4</a>";
	$patlazy[] = "#(^|[\n ])((www|ftp)\.[" . $url_meat . "]{" . $prefix_length . "})([" . $url_meat . "]+)([" . $url_meat . "]{" . $suffix_length . "})($|[^" . $url_meat . "])#is";
	$replazy[] = "\\1<a href=\"http://\\2\\4\\5\\6\" target=\"_blank\">\\2...\\5\\6</a>";
...
MOD-*/
//-- add
function smilies_pass($message, $forum_id = FALSE, $forum_cat_id = FALSE)
...
//-- mod : smiley infobulle ----------------------------------------------------
//-- delete
/*-MOD
			$repl[] = '<img src="'. $board_config['smilies_path'] . '/' . $smilies[$i]['smile_url'] . '" alt="' . $smilies[$i]['emoticon'] . '" border="0" />';
MOD-*/
//-- add
			$repl[] = '<img src="'. $board_config['smilies_path'] . '/' . $smilies[$i]['smile_url'] . '" alt="' . $smilies[$i]['emoticon'] . '"  title="' . $smilies[$i]['emoticon'] . '" style="border:0;" />';
//-- fin mod : smiley infobulle ------------------------------------------------
		}
	}
