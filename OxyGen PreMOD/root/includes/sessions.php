...
//-- modify
	$sql = "UPDATE " . SESSIONS_TABLE . "
		SET session_user_id = $user_id, session_start = $current_time, session_time = $current_time, session_page = $page_id, session_logged_in = $login, is_robot = '$robot', session_admin = $admin
...
//-- modify
		$sql = "INSERT INTO " . SESSIONS_TABLE . "
			(session_id, session_user_id, session_start, session_time, session_ip, session_page, session_logged_in, is_robot, session_admin, session_cf_iso3661_1)
			VALUES ('$session_id', $user_id, $current_time, $current_time, '$user_ip', $page_id, $login, '$robot', $admin, '$iso3661_1')";
//-- fin mod : ip country flag -------------------------------------------------
//-- fin mod : box indexing mod ------------------------------------------------
		if ( !$db->sql_query($sql) )
		{
...
//-- mod : instant msg ---------------------------------------------------------
//-- add
			$sql = 'DELETE FROM ' . INSTANTMSG_TABLE . '
				WHERE date_msg < ' . (time() - $board_config['instant_msg_delay'] * 3600);
			if ( !$db->sql_query($sql) )
			{
				message_die(CRITICAL_ERROR, 'Error clearing messages table', '', __LINE__, __FILE__, $sql);
			}
//-- fin mod : instant msg -----------------------------------------------------
...
//-- modify
			$sql = 'UPDATE ' . USERS_TABLE . " 
				SET user_session_time = $current_time, user_session_page = $page_id, user_lastvisit = $last_visit, user_cf_iso3661_1 = '$iso3661_1'
				WHERE user_id = $user_id";
//-- fin mod : ip country flag -------------------------------------------------

//-- mod : mini last visit -----------------------------------------------------
//-- add
			$sql = str_replace('SET ', 'SET user_lastlogin = ' . $current_time . ', ', $sql);
...
//-- add
	...
	{
		if( $log->config['log_prune_days'] )
		{
			$log_prune_secs = ( $log->config['log_prune_days'] * 24 * 60 * 60 );
			$temp = array(
				'log_user_prune'	=> LOG_TYPE_USER,
				'log_mod_prune'		=> LOG_TYPE_MOD,
				'log_admin_prune'	=> LOG_TYPE_ADMIN,
				'log_error_prune'	=> LOG_TYPE_ERROR,
			);
			...
			if( ( time() - $log->config['log_last_prune'] ) > $log_prune_secs ) // (24 * 60 * 60)
			{
				$log->prune( $log_prune_secs, $log_prune_ary );
			}
		}
	}
//-- fin mod : the logger ------------------------------------------------------
