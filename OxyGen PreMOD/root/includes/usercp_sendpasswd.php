...
//-- add
	...
	{
		$sql = 'SELECT user_id, username, user_regdate, user_email, user_active, user_lang 
			FROM ' . USERS_TABLE . " 
			WHERE user_email = '" . str_replace("\'", "''", $email) . "' 
				AND username = '" . str_replace("\'", "''", $username) . "'";
	}
...
//-- add
			...
			{
				$sql = 'UPDATE ' . USERS_TABLE . " 
					SET user_newpasswd = '" . md5(md5($user_password) . $row['user_regdate']) . "', user_actkey = '$user_actkey'  
					WHERE user_id = " . $row['user_id'];
			}
			else
			{
