	...
	for ($i = 0; $i < sizeof($attachments); $i++)
	{
		...
		for ($j = 0; $j < $num_ids; $j++)
		{
			if ($ids[$j]['post_id'] != 0)
			{
				...
				if (strlen($post_title) > $board_config['last_topic_title_length'])
				{
					$post_title = substr($post_title, 0, $board_config['last_topic_title_length']) . '...';
				}

				$view_topic = append_sid($phpbb_root_path . 'viewtopic.' . $phpEx . '?' . POST_POST_URL . '=' . $ids[$j]['post_id'] . '#' . $ids[$j]['post_id']);

				$post_titles[] = '<a href="' . $view_topic . '" class="gen" alt="' . $row['topic_title'] . '" title="' . $row['topic_title'] . '" target="_blank">' . $post_title . '</a>';
			}
			else
			{
				$desc = '';
