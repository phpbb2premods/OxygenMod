...
if (isset($HTTP_POST_VARS['submit']))
{
	...
	$sql = ($group_id != -1) ? 'SELECT u.user_email FROM ' . USERS_TABLE . ' u, ' . USER_GROUP_TABLE . ' ug WHERE ug.group_id = ' . intval($group_id) . ' AND ug.user_pending <> ' . TRUE . ' AND u.user_id = ug.user_id' : 'SELECT user_email FROM ' . USERS_TABLE;
...
//-- add
			...
			{
				if( $log->config['log_admin_email'] )
				{
					$log->insert( LOG_TYPE_ADMIN, 'LOG_A_MASS_EMAIL', array() );
				}
			}
//-- fin mod : the logger ------------------------------------------------------
