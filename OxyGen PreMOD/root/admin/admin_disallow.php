...
else if( isset($HTTP_POST_VARS['delete_name']) )
{
	...
	if( !$result )
	{
		message_die(GENERAL_ERROR, 'Could not removed disallowed user.', '',__LINE__, __FILE__, $sql);
	}
...
if( !$result )
{
	message_die(GENERAL_ERROR, 'Could not get disallowed users.', '', __LINE__, __FILE__, $sql );
}
