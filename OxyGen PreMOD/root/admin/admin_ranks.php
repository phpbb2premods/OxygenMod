...
if( $mode != '' )
{
	if( $mode == 'edit' || $mode == 'add' )
	{
		...
		if( $mode == 'edit' )
		{
			...
			{
				message_die(GENERAL_ERROR, 'Could not obtain rank data', '', __LINE__, __FILE__, $sql);
			}
	...
	else if( $mode == 'save' )
	{
		...
		if( !$result = $db->sql_query($sql) )
		{
			message_die(GENERAL_ERROR, 'Could not update/insert into ranks table', '', __LINE__, __FILE__, $sql);
		}
	...
	else if( $mode == 'delete' )
	{
		...
		if( $rank_id && $confirm )
		{
			...
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete rank data', '', __LINE__, __FILE__, $sql);
			}
...
if( !$result = $db->sql_query($sql) )
{
	message_die(GENERAL_ERROR, 'Couldnot obtain ranks data', '', __LINE__, __FILE__, $sql);
}
...
//-- mod : rank image display --------------------------------------------------
//-- add
	'L_RANK_IMG' => $lang['Rank_img'],
//-- fin mod : rank image display ----------------------------------------------
...
//-- mod : rank image display --------------------------------------------------
//-- add
		'RANK_IMG' => ( $rank_img != "" ) ? '<img src="../' . $rank_img . '" />' : "",
//-- fin mod : rank image display ----------------------------------------------
