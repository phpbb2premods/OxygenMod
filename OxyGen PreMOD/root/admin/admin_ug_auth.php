...
//	, 'auth_bump'
//-- modify
$forum_auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_bump', 'auth_delete', 'auth_sticky', 'auth_global_announce', 'auth_announce', 'auth_vote', 'auth_pollcreate');
//-- fin mod : bump topic ------------------------------------------------------
...
if ( isset($HTTP_POST_VARS['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
{
	...
	if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'admin' && $user_level != ADMIN )
	{
		...
		if ( $userdata['user_id'] != $user_id )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, 'Could not delete auth access info', '', __LINE__, __FILE__, $sql);
			}
...
//-- modify
			$sql = 'UPDATE ' . AUTH_ACCESS_TABLE . '
				SET auth_view = 0, auth_read = 0, auth_post = 0, auth_reply = 0, auth_edit = 0, auth_delete = 0, auth_sticky = 0, auth_announce = 0, auth_global_announce = 0
				WHERE group_id = ' . $group_id;
//-- fin mod : annonce globale -------------------------------------------------
			if ( !($result = $db->sql_query($sql)) )
			{
...
MOD-*/
//-- add
		$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_userlist'], '<a href="' . append_sid('admin_userlist.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.'.$phpEx . '?pane=right') . '">', '</a>');
//-- fin mod : admin userlist --------------------------------------------------
...
//-- delete
/*-MOD
			$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_userauth'], '<a href="' . append_sid("admin_1ug_auth.$phpEx?mode=$mode") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');
MOD-*/
//-- add
			$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_userlist'], '<a href="' . append_sid('admin_userlist.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.'.$phpEx . '?pane=right') . '">', '</a>');
//-- fin mod : admin userlist --------------------------------------------------
		...
		else
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, 'Could not obtain user/group permissions', '', __LINE__, __FILE__, $sql);
			}
			...
			while( list($forum_id, $action) = @each($forum_auth_action) )
			{
				...
				else
				{
					...
					if( !($result = $db->sql_query($sql)) )
					{
						message_die(GENERAL_ERROR, 'Could not update private forum permissions', '', __LINE__, __FILE__, $sql);
					}
				}
			}
			...
			if ( $delete_sql != '' )
			{
				...
				if( !($result = $db->sql_query($sql)) )
				{
					message_die(GENERAL_ERROR, 'Could not delete permission entries', '', __LINE__, __FILE__, $sql);
				}
			}
...
//-- add
			...
			{
				$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_groupauth'], '<a href="' . append_sid('admin_ug_auth.'.$phpEx . '?mode=' . $mode) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.'.$phpEx . '?pane=right') . '">', '</a>');
			}
//-- fin mod : admin userlist --------------------------------------------------
