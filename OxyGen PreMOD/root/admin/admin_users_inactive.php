...
/**
*
* @version $Id: admin_users_inactive.php,v 1.1.9 2007/04/01 01:12:45 ABDev Exp $
* @copyright (c) 2007 ABDev, EzCom
* @license http://opensource.org/licenses/gpl-license.php GNU Public License
*
* Original author : Sko22, sko22@quellicheilpc.it, 2003
*/
...
$script_name = (!empty($script_name)) ? $script_name . '/profile.' . $phpEx : 'profile.' . $phpEx;
...
if(!isset($HTTP_POST_VARS['inactive_users_do']))
{
	...
	$zero_messages = isset( $HTTP_POST_VARS['zero_messages'] ) ? 'checked="checked"' : '';

	$template->set_filenames(array('body' => 'admin/user_inactive_body.tpl'));
	...
	$sql = "SELECT user_id, username, user_lastvisit, user_regdate, user_posts, user_email, user_active, user_inactive_emls, user_inactive_last_eml
		FROM " . USERS_TABLE . "
		WHERE (user_active = 0 AND user_actkey <> '' $sql_zero_messages)
...
//-- fin mod : rank color system -----------------------------------------------
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$last_user_visit = (!$row['user_lastvisit']) ?  '- -' : create_date($lang['DATE_FORMAT'], $row['user_lastvisit'], $board_config['board_timezone']);
		$last_email_sents = (!$row['user_inactive_last_eml']) ? '- -' : create_date($lang['DATE_FORMAT'], $row['user_inactive_last_eml'], $board_config['board_timezone']);
...
//-- fin mod : rank color system -----------------------------------------------
	...
	if ($i != 0)
	{
		...
		{
			var elts = (typeof(document.forms[the_form].elements['selected_usr[]']) != 'undefined') ? document.forms[the_form].elements['selected_usr[]'] : document.forms[the_form].elements = '';
	...
	switch ($board_config['require_activation'])
	{
		case 0:
			...
			$recom_acc_act = sprintf($lang['UI_Check_Recom'], '<a href="' . append_sid('admin_board.' . $phpEx) . '">', '</a>');
			break;
		case 1:
...
else
{
	...
	{
		case 'contact':
			if (!empty($HTTP_POST_VARS['selected_usr']))
			{
				$date = time();
				...
				foreach ($HTTP_POST_VARS['selected_usr'] as $to_userdata)
				{
					...
					$sql = 'UPDATE ' . USERS_TABLE . '
						SET user_inactive_emls = user_inactive_emls+1, user_inactive_last_eml = ' . $date . '
					...
					//
					$sql = 'SELECT username, user_lang, user_email, user_actkey
						FROM ' . USERS_TABLE . '
						WHERE user_id = ' . $to_userdata . '
							AND user_id <> ' . ANONYMOUS;
					if ($result = $db->sql_query($sql))
					...
					{
						message_die(GENERAL_ERROR, 'Could not send emails to inactive users', '', __LINE__, __FILE__, $sql);
					}
				}

				$message = $lang['Message_sent'] . '<br /><br />' . sprintf($lang['UI_return'], '<a href="' . append_sid('admin_users_inactive.' . $phpEx . '?mode=modify') . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.' . $phpEx . '?pane=right') . '">', '</a>');
		...
		case 'activate':
			...
			if (!empty($HTTP_POST_VARS['selected_usr']))
			{
				...
				foreach ($HTTP_POST_VARS['selected_usr'] as $to_userdata)
				{
					...
					{
						message_die(GENERAL_ERROR, 'Could not send emails to activate users', '', __LINE__, __FILE__, $sql);
					}
				}

				$message = $lang['UI_Activated_Users'] . '<br /><br />' . sprintf($lang['UI_return'], '<a href="' . append_sid('admin_users_inactive.' . $phpEx . '?mode=modify') . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.' . $phpEx . '?pane=right') . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
			} 
			else 
			{
		...
		case 'delete':
			if (!empty($HTTP_POST_VARS['selected_usr']))
			{
				...
				foreach ($HTTP_POST_VARS['selected_usr'] as $to_userdata)
				{
					...
					$sql = 'DELETE FROM ' . USERS_TABLE . ' WHERE user_id = ' . $to_userdata;
					if (!$db->sql_query($sql))
					{
						message_die(GENERAL_ERROR, 'Could not delete user', '', __LINE__, __FILE__, $sql);
					}

					$sql = 'DELETE FROM ' . USER_GROUP_TABLE . ' WHERE user_id = ' . $to_userdata;
					if (!$db->sql_query($sql))
					{
					...
					$sql = 'DELETE FROM ' . TOPICS_WATCH_TABLE . ' WHERE user_id = ' . $to_userdata;
					if (!$db->sql_query($sql))
					{
						message_die(GENERAL_ERROR, 'Could not delete user from topic watch table', '', __LINE__, __FILE__, $sql);
					}

					$sql = 'DELETE FROM ' . BANLIST_TABLE . ' WHERE ban_userid = ' . $to_userdata;
					if (!$db->sql_query($sql))
					{
