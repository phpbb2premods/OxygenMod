...
//-- mod : presentation --------------------------------------------------------
//-- add
if ($board_config['present_required'])
{
	$sql = 'SELECT * FROM ' . TOPICS_TABLE . ' WHERE topic_poster = ' . intval($userdata['user_id']) . ' AND forum_id = ' . $board_config['present_forum'];
	...
	if (empty($row['topic_id']))
	{
		...
		$row = $db->sql_fetchrow($result);
		$present_forum = empty($forum_id) ? $row['forum_id'] : $forum_id;

		if ($present_forum != $board_config['present_forum'])
		{
			if ($userdata['user_level'] != ADMIN)
			{
				$message = $lang['presus'] . '<br /><br />' . sprintf($lang['presuscli'], '<a href="' . append_sid('viewforum.' . $phpEx . '?f=' . $board_config['present_forum']) . '">', '</a> ');
...
//-- add
		...
		$sql = 'SELECT f.*, t.topic_status, t.topic_title, t.topic_type, t.topic_bumped, t.topic_last_post_time, t.topic_poster, p.poster_id
...
//-- add
		if( !$board_config['disallow_edition_deleting_admin_messages'] && ( $userdata['user_level'] != ADMIN && ($mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete') ) )
		{
...
//	, pt.post_sub_title
//-- modify
		$select_sql = (!$submit) ? ', t.topic_title, t.topic_icon, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, pt.post_subject, pt.post_sub_title, p.post_icon, pt.post_text, pt.bbcode_uid, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid' : '';
...
//-- mod : bump topic ----------------------------------------------------------
//-- add
$bump_time = bump_topic_allowed($post_info['topic_bumped'], $post_info['topic_last_post_time'], $post_info['topic_poster'], $post_info['poster_id']);
$bump = array(
	'is_admin' => $userdata['session_logged_in'] && ($userdata['user_level'] == ADMIN),
	'is_poster' => ($post_info['poster_id'] == $userdata['user_id']),
	'is_bumped' => empty($bump_time) && !empty($board_config['bump_interval']),
);

if ( $mode == 'bump' && !empty($bump_time) )
{
	$current_time = time();
	...
	if ( !$db->sql_query($sql) )
	{
		message_die(GENERAL_ERROR, 'Could not update post time', '', __LINE__, __FILE__, $sql);
	}

	$sql = 'UPDATE ' . TOPICS_TABLE . '
		SET topic_last_post_time = ' . $current_time . ',
			topic_bumped = 1,
			topic_bumper = ' . intval($userdata['user_id']) . '
		WHERE topic_id = ' . $topic_id . '
			AND forum_id = ' . $forum_id;
	if ( !$db->sql_query($sql) )
	{
		message_die(GENERAL_ERROR, 'Could not update topic last post information', '', __LINE__, __FILE__, $sql);
	}

	markread($topic_id, $current_time);

	$template->assign_vars(array(
		'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid('viewtopic.' . $phpEx . '?' . POST_POST_URL . '=' . $topic_last_post_id) . '#' . $topic_last_post_id . '">',
	));

	$message = $lang['topic_bumped'] . '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid('viewtopic.' . $phpEx . '?' . POST_POST_URL . '=' . $topic_last_post_id) . '#' . $topic_last_post_id . '">', '</a>');
...
if ( $mode == 'reply' && ( $bump['is_bumped'] || !empty($board_config['reply_flood_ctrl']) ) && $bump['is_poster'] && !$bump['is_admin'] )
{
	$message = $lang['reply_error_without_bump'] . ( $bump['is_bumped'] ? '<br /><br />' . sprintf($lang['reply_error_with_bump'], $board_config['bump_interval'], $lang['bt_type_' . $board_config['bump_type'] . ( ($board_config['bump_interval'] == 1) ? '_s' : '_p' )]) : '' );
...
//-- add
	if ($mode == 'reply' && $userdata['user_id'] != ANONYMOUS)
	{
		...
		$sql = 'SELECT post_id, poster_id, post_time, MAX(post_time)
			FROM ' . POSTS_TABLE . '
		...
		$separator = "\n\n" . '[size=9][color=#999999]' . $lang['Added_after'];
		$separator .= ($hours) ? (($hours == 1) ? $lang['dpm_hour'] : sprintf($lang['dpm_hours'], $hours)) : '';
		$separator .= ($minutes) ? (($minutes == 1) ? $lang['dpm_minute'] : sprintf($lang['dpm_minutes'], $minutes)) : '';
		$separator .= ($seconds == 1) ? $lang['dpm_second'] : sprintf($lang['dpm_seconds'], $seconds);
		$separator .= ':[/color][/size]' . "\n\n";

		if ($post_id_last_row['poster_id'] == $poster_id && ($difference_time < ($board_config['join_interval'] * 3600)))
		{
			$mode = 'editpost';
			$sql = 'SELECT post_text, bbcode_uid FROM ' . POSTS_TEXT_TABLE . ' WHERE post_id = ' . intval($post_id);
			if (!$result = $db->sql_query($sql))
			{
				message_die(GENERAL_ERROR, 'Could not obtain forums information', '', __LINE__, __FILE__, $sql);
			}
...
//-- add
			if( defined( 'LOG_MOD_INSTALLED' ) )
			{
				if( $mode == 'editpost' )
				{
					if( $log->config['log_mod_edit'] && !$post_data['poster_post'] )
					{
						$log->insert( LOG_TYPE_MOD, 'LOG_M_EDIT', array( 'post_id' => $post_id ), '', $topic_id );
					}
					elseif( $log->config['log_user_edit'] )
					{
						$log->insert( LOG_TYPE_USER, 'LOG_U_EDIT', array( 'post_id' => $post_id ), '', $topic_id );
					}
				}
				elseif( $mode == 'newtopic' )
				{
					if( $log->config['log_user_newtopic'] )
					{
						$log->insert( LOG_TYPE_USER, 'LOG_U_NEWTOPIC', array( $subject ), $forum_id );
					}
				}
			}
//-- fin mod : the logger ------------------------------------------------------
...
//	, $sub_title
//-- modify
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), $sub_title, str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $lock_status, $post_info['topic_status'], $added, $post_icon);
//-- fin mod : post description ------------------------------------------------
//-- fin mod : lock during posting ---------------------------------------------
//-- fin mod : post icon -------------------------------------------------------
//-- fin mod : double post merge -----------------------------------------------
...
//-- add
			if( defined( 'LOG_MOD_INSTALLED' ) )
			{
				if( $mode == 'delete' )
				{
					if( ($log->config['log_mod_delete'] && !$post_data['poster_post']) || $post_data['first_post'] )
					{
						if( $post_data['first_post'] )
						{
							$log_data = array( $log->convert_post_id( $post_id ) );
							$log_topic_id = 0;
							$log_type = 'LOG_M_DELETE_TOPIC';
						}
						else
						{
							$log_data = array();
							$log_topic_id = $topic_id;
							$log_type = 'LOG_M_DELETE';
						}
						
						$log->insert( LOG_TYPE_MOD, $log_type, $log_data, '', $log_topic_id );
					}
					elseif( $log->config['log_user_delete'] )
					{
						$log->insert( LOG_TYPE_USER, 'LOG_U_DELETE', array(), '', $topic_id );
					}
				}
			}
//-- fin mod : the logger ------------------------------------------------------
...
//	, $lock_status
//-- modify
			update_post_stats($mode, $post_data, $forum_id, $topic_id, $post_id, $user_id, $lock_status);
//-- fin mod : lock during posting ---------------------------------------------
		}
...
MOD-*/
//-- add
			$preview_message = smilies_pass($preview_message, $forum_id, $forum_cat_id);
//-- fin mod : smiley categories -----------------------------------------------
		}
...
MOD-*/
//-- add
generate_smilies('inline', PAGE_POSTING, $forum_id, $forum_cat_id);
//-- fin mod : smiley categories -----------------------------------------------
...
$icon_per_row = isset($board_config['icon_per_row']) ? intval($board_config['icon_per_row']) : 8;
if ($icon_per_row <= 1)
{
	$icon_per_row = 10;
}
...
MOD-*/
//-- add
		$template->assign_vars(array('POLL_OPTION' => implode("\n", str_replace('"', '&quot;', $poll_options))));
//-- fin mod : quick poll insert -----------------------------------------------
	}
...
MOD-*/
//-- add
	topic_review($topic_id, true, $forum_id, $forum_cat_id);
//-- fin mod : smiley categories -----------------------------------------------
