...
<p>
	{L_USERS_INACTIVE_EXPLAIN}<br /><br /><u>{L_CHECK_ENABLE_ACCOUNT_ACTIVATION}</u> {L_RECOM_ACC_ACTIV}<br />&nbsp;<br />
	{L_REMOVED_USERS}: <b>{REMOVED_USERS}</b>
</p>
...
<table cellspacing="2" cellpadding="4" border="0">
 <tr>
 	<td colspan="6">{L_ALERT_EVERY} <input class="post" type="text" name="alert_days" value="{S_ALERT_DAYS}" size="3"> {L_ALERT_DAYS} {L_WITH_ZERO_MESSAGES} <input type="checkbox" name="zero_messages" {S_ZERO_MESSAGES}> <input type="submit" name="alert_days_check" value="{L_POST}" class="mainoption" onClick="document.tablesForm.action='{S_INACTIVE_ACTION}'" /></td>
 </tr>
 <tr>
