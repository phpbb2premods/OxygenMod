...
//-- add
		...
		{
			$sql = "SELECT user_id, username, user_password, user_regdate, user_active, user_level, user_login_tries, user_last_login_try
				FROM " . USERS_TABLE . "
				WHERE username = '" . str_replace("\\'", "''", $username) . "'";
//-- mod : restrict account to ip ----------------------------------------------
//-- add
			$sql = str_replace('SELECT ', 'SELECT user_iprange, user_restrictip, ', $sql);
...
MOD-*/
//-- add
				if ( $board_config['password_security_startdate'] && ( md5(md5($password) . $row['user_regdate']) == $row['user_password'] && $row['user_active'] ) )
				{
					$autologin = ( isset($HTTP_POST_VARS['autologin']) ) ? TRUE : 0;
...
//-- add
						...
						{
							if( $log->config['log_user_login'] )
							{
								$log->insert( LOG_TYPE_USER, 'LOG_U_LOGIN', array( $admin ), '', '', $row['user_id'] );
							}
						}
//-- fin mod : the logger ------------------------------------------------------
...
//-- add
				if( defined( 'LOG_MOD_INSTALLED' ) )
				{
					if( $log->config['log_user_fail_login'] )
					{
						$log->insert( LOG_TYPE_USER, 'LOG_U_LOGIN_FAILED', array() );
					}
				}
//-- fin mod : the logger ------------------------------------------------------
...
//-- add
			if( defined( 'LOG_MOD_INSTALLED' ) )
			{
				if( $log->config['log_user_logout'] )
				{
					$log->insert( LOG_TYPE_USER, 'LOG_U_LOGOUT', array() );
				}
			}
//-- fin mod : the logger ------------------------------------------------------
